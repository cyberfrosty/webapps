...
def parse_options():
    ...
    parser.add_argument('command', action='store', help='check, csv, gallery, masonary')
...
def read_csv(csv_file):
    """ Read a CSV file with art piece metadata
    Args:
        csv filename
    Return:
...
def write_csv(images):
    """ Write a CSV file with art piece metadata
    Args:
        images dict
    """
...
def check_images(images):
    """ Check an images dict for required art piece metadata
    Args:
        images dict
    """
    for image in images:
...
def generate_masonary_page(site, page, title, images, medium='*'):
    ...
    for image in images:
        if medium == '*' or medium in image['medium']:
            ...
            html = '      <a href="gallery.html?slide=' + str(slide) + '" title="'
            html += title + ', ' + image['medium'] + ', ' + image['size'] + ', ' + image['created'] + '">'
            print(html)
            html = '      <img src="' + site + 'static/img/' + img_small + '" alt="' + title + '"></a>'
            print(html)
            slide += 1
...
def generate_gallery(site, images, medium='*'):
    ...
    html += '    <div class="container">\n'
    html += '      <div id="artCarousel" class="carousel slide carousel-fade" data-ride="carousel" data-interval="6000">\n'
    print(html)
